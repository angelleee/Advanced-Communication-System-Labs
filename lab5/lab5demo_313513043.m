...
data = load('./Lab5_given_data/S5P1.mat');
bs = data.bs(:).';
L = length(bs);
...
for i = 1:Nsymbols
    seg = bs((i-1)*n+1 : i*n);
    segVal = seg * (2.^(n-1:-1:0)).';

    idx = find(segVal == grayCodes);
    symbols(i) = levels(idx);
end

figure;
stem(1:Nsymbols, symbols);
grid on;

%% p2
clc; clear; close all;

data = load('./Lab5_given_data/S5P2.mat');
bs = data.bs(:).';
L = length(bs);
